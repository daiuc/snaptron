...
params:
    ...
    denom_cutoff: 30
    ir_dist_cutoff: 0.1
    gtex_tpm: "../code/resources/GTEx-downloads/gene_tpm_2017-06-05_v8_muscle_skeletal.gct.gz" # gtex rna-seq tpm matrix
...
quantileHighPass = function(v, quantile.threshold = .95, sd.multiplier = 1){
  ...
  pass.flag <- mean(higher.v) >= sd.multiplier * lower.sd + quantile(v, quantile.threshold)
  return(pass.flag)
}

# on the flip side
quantileLowPass = function(v, quantile.threshold = .05, sd.multiplier = 1){
  higher.v = v[which(v >= quantile(v, quantile.threshold))]
  higher.sd = sd(higher.v)
  
  lower.v = v[which(v < quantile(v, quantile.threshold))]
  
  # if the median of 96th-100th is greather than 2*SD away from the 95th quantile, then pass
  pass.flag <- mean(lower.v) <= sd.multiplier * higher.sd + quantile(v, quantile.threshold)
  return(pass.flag)
}
...
# not used 
# quantilePass2 = function(dt, quantile.threshold = .95, sd.multiplier = 1){
#   dt = dt[denom > 30]
#   v = dt[, ir]
#   lower.v = v[which(v <= quantile(v, quantile.threshold))]
#   lower.sd = sd(lower.v)
...
```{r}
# read in gtex tpm
gtex.tpm = fread(params$gtex_tpm)
```
...
p2 = outlier_introns[chrom %in% select_chroms] %>% 
  ggplot() + geom_boxplot(aes(fct_reorder(chrom, p, min), ir), outlier.alpha = 0) + 
...
- Specifically, an outlier intron is qualified only if the mean excision ratio of samples in the (95, 100] percentile is more than 1x sd away from the 95th percentile, where `sd` is estimated by (0,95] percentiles. 
- Equivalently, outliers in the lower end is also examined using the 5th percentile. 
...
```{r}
outlier_introns = outlier_introns[!samples %in% excl_samples]

outlier_introns[, `:=`(
  highPass = quantileHighPass(ir),
  lowPass = quantileLowPass(ir),
  pct_rank = percent_rank(ir)
), by = chrom]

outlier_introns[, `:=`(
  outlier2 = fcase(outlier == T & (pct_rank > .75 | pct_rank < .25), TRUE, default = F)
)]

# called outlier introns must be pass either the high or low scenario for quantile filters.
qtPassed_chroms = intersect(outlier_introns[outlier == T, chrom], outlier_introns[highPass == T | lowPass == T, chrom])
```


There are `r length(qtPassed_chroms)` outlier introns after this round of quantile based filter. The box plot below randomly select 50 out of the outlier introns (x is sorted by p value). 

In summary, the two box plots below plotted some of the outlier introns that satisfy these criteria: 

  1.  p-value of a given intron excision event \< `r scales::scientific(p_cutoff)`
  2.  **AND** total reads of the cluster \> `r denom_cutoff`
  3. **AND** intron excision ratio is at least `r params$ir_dist_cutoff` away from median
  4. **AND** outlier intron must pass quantile filter described above
  5. **AND** an outlier event in a given sample must be in the top or bottom 5 percentiles in terms of intron excision ratio
  6. **AND** exclude outliers from extreme samples
...
outlier_introns[chrom %in% x] %>% 
  ggplot() + geom_boxplot(aes(fct_reorder(chrom, p, min), ir, group = chrom), outlier.alpha = 0) + 
    geom_point(aes(fct_reorder(chrom, p, min), ir, color = outlier2, shape = outlier2), 
               alpha = .4, position = position_jitter(height = 0)) +
    labs(x = NULL, y = "Intron excision ratio") + 
    guides(color = guide_legend(override.aes = list(alpha = 1, size = 2))) + 
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90))
...
```{r, fig.width=10, fig.height=6}
outlier_introns[chrom %in% x] %>% 
  ggplot() + geom_boxplot(aes(fct_reorder(chrom, p, min), rc), outlier.alpha = 0) + 
    geom_point(aes(fct_reorder(chrom, p, min), rc, color = outlier2, shape = outlier2), alpha = .4, position = "jitter") +
    scale_y_log10() + 
    guides(color = guide_legend(override.aes = list(alpha = 1, size = 2))) + 
    labs(x = NULL, y = "reads") + 
    theme_bw() +
...
outlier per sample after all filters.

```{r}
outlier_introns[ chrom %in% qtPassed_chroms & outlier == T & !samples %in% excl_samples][, .(N = length(unique(chrom))), by = samples] %>% 
  ggplot()  + geom_col(aes(fct_reorder(samples, -N), N, fill = samples)) + 
    labs(x = NULL) + 
    scale_y_log10() + 
    theme(legend.position = "None", axis.text.x = element_blank())
...
Number of outlier samples per outlier intron
```{r}
outlier_introns[ chrom %in% qtPassed_chroms & outlier == T & !samples %in% excl_samples][, .(N = length(unique(samples))), by = chrom] %>% 
